#-----------------------------------------------------------------------------------------------------#
#   Simulation Studies: Sample Simulation Script for Frailty Misspecification Sensitivity Analysis    #
#-----------------------------------------------------------------------------------------------------#
...
#        construct the projected survival functions;
#     3) the estimated conditional power, assuming that the components of the projected survival 
#        functions are estimated from/informed by the interim data under correct specification of the 
#        gamma frailty distribution; and
#     4) the estimated conditional power, assuming that the components of the projected survival
#        functions are estimated from/informed by the interim data under misspecification of the 
#        frailty distribution as lognormal
...
#     b) a mild clustering effect (\sigma^2 = 0.06 <==> \theta = 1/{exp(0.06)-1}),
...
# 125 times; the full sensitivity analysis considers three such simulation settings. Wherever possible, 
# I have marked where the variables in the sample simulation script may be changed in order to obtain the 
# full range of data-generating mechanisms and CRT designs considered in the supporting information.
...
one_run <- function(M, ni, hr, t.dist, t.theta, f.dist, f.theta, start.delta, visit.times, visit.delta,
  ...
  # Calculating conditional power: point-estimated hazard, HR, and frailties (under gamma frailty model)
  est.cp.gamma <- est_cp(int.data, interim, visit.times, visit.delta, l0.delta, l1.delta, p_value, 0.05, 500, 
                         f.dist=f.dist, trunc.time=c(visit.times[interim.visit], visit.times[length(visit.times)]))
  
  # Calculating conditional power: point-estimated hazard, HR, and frailties (under lognormal frailty model)
  est.cp.ln <- est_cp(int.data, interim, visit.times, visit.delta, l0.delta, l1.delta, p_value, 0.05, 500, 
                      f.dist="lognormal", trunc.time=c(visit.times[interim.visit], visit.times[length(visit.times)]))
  ...
  res <- data.frame(M = M, t.dist = t.dist, hazard = t.theta, hr=hr, f.dist=f.dist, f.theta = f.theta, cens=cens,
                    ...
                    int.f.theta = est.cp.gamma$int.theta, int.cens = int.cens, int.events.0 = est.cp.gamma$int.events[1], 
                    int.pt.0 = est.cp.gamma$int.person.time[1], int.hazard.0 = est.cp.gamma$int.hazard$control, 
                    int.events.1 = est.cp.gamma$int.events[2], int.pt.1 = est.cp.gamma$int.person.time[2], 
                    int.hazard.1 = est.cp.gamma$int.hazard$treat, int.hr=est.cp.gamma$int.hr, 
                    
                    l0.delta = l0.delta, l1.delta = l1.delta, power = est.power$power, 
                    
                    gamma.cp = est.cp.gamma$cp, gamma.proj.mean.f.theta = mean(est.cp.gamma$full.theta), 
                    gamma.proj.min.f.theta = min(est.cp.gamma$full.theta), gamma.proj.max.f.theta = max(est.cp.gamma$full.theta), 
                    gamma.proj.var.f.theta = var(est.cp.gamma$full.theta), gamma.proj.mse.f.theta.int = mean((est.cp.gamma$full.theta-est.cp.gamma$int.theta)^2), 
                    gamma.proj.mse.f.theta.actual = mean((est.cp.gamma$full.theta-actual.f[[2]])^2), gamma.proj.mean.cens = mean(est.cp.gamma$full.cens),
                    gamma.proj.mean.events.0 = mean(est.cp.gamma$full.events[,1]), gamma.proj.min.events.0 = min(est.cp.gamma$full.events[,1]),
                    gamma.proj.max.events.0 = max(est.cp.gamma$full.events[,1]), gamma.proj.var.events.0 = var(est.cp.gamma$full.events[,1]),
                    gamma.proj.mean.pt.0 = mean(est.cp.gamma$full.person.time[,1]), gamma.proj.min.pt.0 = min(est.cp.gamma$full.person.time[,1]),
                    gamma.proj.max.pt.0 = max(est.cp.gamma$full.person.time[,1]), gamma.proj.var.pt.0 = var(est.cp.gamma$full.person.time[,1]),
                    gamma.proj.mean.hazard.0 = mean(est.cp.gamma$full.hazard$control), gamma.proj.min.hazard.0 = min(est.cp.gamma$full.hazard$control),
                    gamma.proj.max.hazard.0 = max(est.cp.gamma$full.hazard$control), gamma.proj.var.hazard.0 = var(est.cp.gamma$full.hazard$control),
                    gamma.proj.mse.hazard.0.int = mean((est.cp.gamma$full.hazard$control - est.cp.gamma$int.hazard$control)^2), 
                    gamma.proj.mse.hazard.0.actual = mean((est.cp.gamma$full.hazard$control-actual.hazard.0)^2),
                    gamma.proj.mean.events.1 = mean(est.cp.gamma$full.events[,2]), gamma.proj.min.events.1 = min(est.cp.gamma$full.events[,2]),
                    gamma.proj.max.events.1 = max(est.cp.gamma$full.events[,2]), gamma.proj.var.events.1 = var(est.cp.gamma$full.events[,2]),
                    gamma.proj.mean.pt.1 = mean(est.cp.gamma$full.person.time[,2]), gamma.proj.min.pt.1 = min(est.cp.gamma$full.person.time[,2]),
                    gamma.proj.max.pt.1 = max(est.cp.gamma$full.person.time[,2]), gamma.proj.var.pt.1 = var(est.cp.gamma$full.person.time[,2]),
                    gamma.proj.mean.hazard.1 = mean(est.cp.gamma$full.hazard$treat), gamma.proj.min.hazard.1 = min(est.cp.gamma$full.hazard$treat),
                    gamma.proj.max.hazard.1 = max(est.cp.gamma$full.hazard$treat), gamma.proj.var.hazard.1 = var(est.cp.gamma$full.hazard$treat),
                    gamma.proj.mse.hazard.1.int = mean((est.cp.gamma$full.hazard$treat - est.cp.gamma$int.hazard$treat)^2), 
                    gamma.proj.mse.hazard.1.actual = mean((est.cp.gamma$full.hazard$treat-actual.hazard.1)^2),
                    gamma.proj.mean.hr = mean(est.cp.gamma$full.hr), gamma.proj.geom.mean.hr = exp(mean(log(est.cp.gamma$full.hr))),
                    
                    ln.cp = est.cp.ln$cp, ln.proj.mean.f.theta = mean(est.cp.ln$full.theta), 
                    ln.proj.min.f.theta = min(est.cp.ln$full.theta), ln.proj.max.f.theta = max(est.cp.ln$full.theta), 
                    ln.proj.var.f.theta = var(est.cp.ln$full.theta), ln.proj.mse.f.theta.int = mean((est.cp.ln$full.theta-est.cp.ln$int.theta)^2), 
                    ln.proj.mse.f.theta.actual = mean((est.cp.ln$full.theta-actual.f[[2]])^2), ln.proj.mean.cens = mean(est.cp.ln$full.cens),
                    ln.proj.mean.events.0 = mean(est.cp.ln$full.events[,1]), ln.proj.min.events.0 = min(est.cp.ln$full.events[,1]),
                    ln.proj.max.events.0 = max(est.cp.ln$full.events[,1]), ln.proj.var.events.0 = var(est.cp.ln$full.events[,1]),
                    ln.proj.mean.pt.0 = mean(est.cp.ln$full.person.time[,1]), ln.proj.min.pt.0 = min(est.cp.ln$full.person.time[,1]),
                    ln.proj.max.pt.0 = max(est.cp.ln$full.person.time[,1]), ln.proj.var.pt.0 = var(est.cp.ln$full.person.time[,1]),
                    ln.proj.mean.hazard.0 = mean(est.cp.ln$full.hazard$control), ln.proj.min.hazard.0 = min(est.cp.ln$full.hazard$control),
                    ln.proj.max.hazard.0 = max(est.cp.ln$full.hazard$control), ln.proj.var.hazard.0 = var(est.cp.ln$full.hazard$control),
                    ln.proj.mse.hazard.0.int = mean((est.cp.ln$full.hazard$control - est.cp.ln$int.hazard$control)^2), 
                    ln.proj.mse.hazard.0.actual = mean((est.cp.ln$full.hazard$control-actual.hazard.0)^2),
                    ln.proj.mean.events.1 = mean(est.cp.ln$full.events[,2]), ln.proj.min.events.1 = min(est.cp.ln$full.events[,2]),
                    ln.proj.max.events.1 = max(est.cp.ln$full.events[,2]), ln.proj.var.events.1 = var(est.cp.ln$full.events[,2]),
                    ln.proj.mean.pt.1 = mean(est.cp.ln$full.person.time[,2]), ln.proj.min.pt.1 = min(est.cp.ln$full.person.time[,2]),
                    ln.proj.max.pt.1 = max(est.cp.ln$full.person.time[,2]), ln.proj.var.pt.1 = var(est.cp.ln$full.person.time[,2]),
                    ln.proj.mean.hazard.1 = mean(est.cp.ln$full.hazard$treat), ln.proj.min.hazard.1 = min(est.cp.ln$full.hazard$treat),
                    ln.proj.max.hazard.1 = max(est.cp.ln$full.hazard$treat), ln.proj.var.hazard.1 = var(est.cp.ln$full.hazard$treat),
                    ln.proj.mse.hazard.1.int = mean((est.cp.ln$full.hazard$treat - est.cp.ln$int.hazard$treat)^2), 
                    ln.proj.mse.hazard.1.actual = mean((est.cp.ln$full.hazard$treat-actual.hazard.1)^2),
                    ln.proj.mean.hr = mean(est.cp.ln$full.hr), ln.proj.geom.mean.hr = exp(mean(log(est.cp.ln$full.hr))),
                    ...
                    known.var.f.theta = var(known.cp$full.theta), known.mse.f.theta.int = mean((known.cp$full.theta-est.cp.gamma$int.theta)^2), 
                    ...
                    known.mse.hazard.0.int = mean((known.cp$full.hazard$control - est.cp.gamma$int.hazard$control)^2), 
                    ...
                    known.mse.hazard.1.int = mean((known.cp$full.hazard$treat - est.cp.gamma$int.hazard$treat)^2), 
...
one_run <- function(M, ni, hr, t.dist, t.theta, f.dist, f.theta, start.times, start.delta, visit.times, visit.delta,
  ...
  # Determining the study entry times and final analysis method
  if (paired){
    p_value <- matched_analysis
  } else {
    p_value <- unmatched_analysis
...
t.theta <- 0.001            
f.dist <- c("gamma")
f.theta <- 1/(exp(0.06)-1)            
hr <- exp(0)                ## Full Simulation Study: hr %in% c(exp(-0.4), exp(-0.2), exp(0))
