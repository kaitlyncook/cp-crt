...
est_curve <- function(data, f.dist, f.theta, marginal=FALSE){
  ...
  if (cntrl.fit[nrow(cntrl.fit), 2] == Inf){
    cntrl.fit[nrow(cntrl.fit), 2] <- cntrl.fit[nrow(cntrl.fit), 1]
  }
  cntrl.fit <- rbind(cntrl.fit, c(Inf, Inf, cntrl.fit[nrow(cntrl.fit), 3]))
  if (tx.fit[nrow(tx.fit), 2] == Inf){
    tx.fit[nrow(tx.fit), 2] <- tx.fit[nrow(tx.fit), 1]
  }
  tx.fit <- rbind(tx.fit, c(Inf, Inf, tx.fit[nrow(tx.fit), 3]))
...
est_hazard <- function(s.curve, end.point = NULL){
  ...
  get_surv <- function(t){
    lower.index <- length(which(s.curve[, 1] <= t))
    upper.index <- which(s.curve[, 2] > t)[1]
    if (lower.index==upper.index){
      v.lower <- c(s.curve[lower.index, 1], s.curve[lower.index-1, 3])
      v.upper <- c(s.curve[upper.index, 2], s.curve[upper.index, 3])
      s <- ((v.upper[2]-v.lower[2])/(v.upper[1]-v.lower[1]))*t + (v.lower[2]*v.upper[1] - v.upper[2]*v.lower[1])/(v.upper[1]-v.lower[1])
    } else {
      s <- s.curve[lower.index, 3]
    }
  }
...
proj_observation <- function(left, interim, visit.times, visit.delta, s.curve, hazard, eta, cens){
  ...
  get_surv <- function(t){
    t.inf <- min(left, s.curve[nrow(s.curve)-1, 1])
    if (t <= t.inf){
      lower.index <- length(which(s.curve[, 1] <= t))
      upper.index <- which(s.curve[, 2] > t)[1]
      if (lower.index==upper.index){
      ...
      inf.lower.index <- length(which(s.curve[, 1] <= t.inf))
      inf.upper.index <- which(s.curve[, 2] > t.inf)[1]
      if(inf.lower.index==inf.upper.index){
        inf.lower <- c(s.curve[inf.lower.index, 1], s.curve[inf.lower.index-1, 3])
        inf.upper <- c(s.curve[inf.upper.index, 2], s.curve[inf.upper.index, 3])
        s.inf <- ((inf.upper[2]-inf.lower[2])/(inf.upper[1]-inf.lower[1]))*t.inf + (inf.lower[2]*inf.upper[1] - inf.upper[2]*inf.lower[1])/(inf.upper[1]-inf.lower[1])
      } else{
        s.inf <- s.curve[lower.index, 3]
      }
  ...
  get_inv_surv <- function(s){
    ...
    if (s >= s.inf){
      index <- which(s.curve[, 3] <= s)[1]
      if (s.curve[index, 3]==s){
        t <- s.curve[index, 2]
      } else {
        v.lower <- c(s.curve[index, 1], s.curve[index-1, 3])
        v.upper <- c(s.curve[index, 2], s.curve[index, 3])
        t <- v.upper[1] + (s - v.upper[2])*(v.lower[1] - v.upper[1])/(v.lower[2]-v.upper[2])
      }
    } else {
