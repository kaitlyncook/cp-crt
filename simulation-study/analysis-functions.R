...
visualize_data_gen <- function(data, beta){
  ...
  for (i in 1:n.breaks){
    ...
    x.beta <- format(round(unique(b.dat$beta[b.dat$x.pos==x.name]), 2), nsmall=2)
    x.labels[i] <- ifelse(nchar(x.beta)==5, paste0(x.setting, ": ", x.beta), paste0(x.setting, ":   ", x.beta))
...
visualize_accuracy_matched <- function(matched.data, unmatched.data, alpha=0.05, threshold=0.2){
  ...
  accuracy.dat$accuracy[is.na(accuracy.dat$accuracy)] <- 1
  ...
  p <- p + scale_fill_viridis(name="Accuracy", breaks=c(0.8, 0.9, 1), direction=-1, begin=0, end=0.9) + facet_wrap(~matched, nrow=2)
...
visualize_accuracy_incidence <- function(data, alpha=0.05, threshold=0.2){
  ...
  p <- p + scale_fill_viridis(name="Accuracy", direction=-1, begin=0, end=0.9) + facet_wrap(~lambda, nrow=2, labeller = label_parsed)
...
### Computes the absolute difference between two curves at time t
